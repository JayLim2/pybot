...
class SiameseBiLSTM:
    ...
    def train_model(self, sentences_pair, is_similar, embedding_meta_data, model_save_directory='./'):
        ...
        best_model_path_value = checkpoint_dir + STAMP + '.h5'

        model_checkpoint = ModelCheckpoint(best_model_path_value, save_best_only=True, save_weights_only=False)
        ...
        model.fit([train_data_x1, train_data_x2, leaks_train], train_labels,
                  ...
                  callbacks=[model_checkpoint, tensorboard]) # early_stopping deleted from here

        last_checkpoint = model_save_directory + "checkpoints/last_checkpoint.txt"
        with open(last_checkpoint, 'w') as last_checkpoint_out:
            last_checkpoint_out.write(best_model_path_value)
            last_checkpoint_out.flush()

        return best_model_path_value
    ...
    def update_model(self, saved_model_path, new_sentences_pair, is_similar, embedding_meta_data):
        ...
        model.fit([train_data_x1, train_data_x2, leaks_train], train_labels,
                  ...
                  callbacks=[early_stopping, model_checkpoint, tensorboard]) # early_stopping deleted from here
